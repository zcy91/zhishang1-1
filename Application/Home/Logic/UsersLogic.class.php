...
{    /*
    ...
    public function reg($username,$password,$password2){
        ...
        $map['reg_time'] = time();
        $map['first_leader'] =$_COOKIE['t_id']?$_COOKIE['t_id']:1; // 推荐人id
        $map['is_distribut']  =0;//$_COOKIE['shang']=="jia"?1:0; // 是否为商家
        ...
        if(!$user_id){       
    		...
    		if($pay_points > 0){accountLog($user_id,$pay_points,'pay_points' , '赠送积分');} // 会员注册赠送积分
...
//        $type = I('get.type',0);
        ...
        $count = M('account_log')->where($where ? $where : 'user_id = '.$user_id.' and user_money!=0')->count();
        $Page = new Page($count,16);
        $logs = M('account_log')->where($where ? $where : 'user_id = '.$user_id.' and user_money!=0')->order('change_time desc')->limit($Page->firstRow.','.$Page->listRows)->select();
...
//        	}
     ...
     * @return array
     */
    public function add_zwzq($user_id,$address_id=0,$data){
        $post = $data;      
        //检查格式
        if($post['title'] == '')
            return array('status'=>-1,'msg'=>'平台名称或关联人姓名不能为空','result'=>'');
        if(!$post['money'])
            return array('status'=>-1,'msg'=>'受损金额不能为空','result'=>'');
        if(!$post['site'] and $post['title']=="平台类")
            return array('status'=>-1,'msg'=>'平台网址不能为空','result'=>'');
    

        //添加模式
        $post['user_id'] = $user_id;
        $post['add_time'] =time();
		$post['title_t'] =getFirstCharter($post['title']);
		if(tpCache('distribut.right_switch') &&  tpCache('distribut.debt_amount')){//自动处理
			 $post['amount']=$post['money']*tpCache('distribut.debt_amount');
			 $post['zt']=1;
			 $post['end_time'] =time();
			 $post['bz']="自动审核";
		}
		
        $address_id = M('user_zwzq')->add($post);    
        if($address_id){
			  if(tpCache('distribut.right_switch') &&  tpCache('distribut.debt_amount')){//自动处理
				  accountLog($user_id,$post['amount'],'dai_points' , "发放额度",$post['money'],0,'债权'.$post['money'].'元,发放'.$post['amount'].'额度'); // 增加额度
			  }
           return array('status'=>1,'msg'=>'添加成功','result'=>$address_id); 
		}
        return array('status'=>-1,'msg'=>'添加失败','result'=>'');
...
//        }
     ...
     * @return json
     */
    public function send_validate_code($sender,$type,$session_id=0){
    	$sms_time_out = tpCache('sms.sms_time_out');
    	...
    	if($type == 'email'){
    		...
    		if(!check_mobile($sender))
    		    ...
    		    $send = sendSMS($sender,"【至上网络】您的验证码是{$code}");
    	}
        //$send =1;
    	if($send){
    		$info['code'] = $code;
			...
			M('sms_log')->add(array('mobile'=>$sender,'code'=>$code,'add_time'=>time(),'session_id'=>$session_id));
    		session('validate_code',$info);
    		$res = array('status'=>1,'msg'=>'验证码已发送，请注意查收');//.$code
    	}else{
