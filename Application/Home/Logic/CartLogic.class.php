...
class CartLogic extends RelationModel
{
    ...
    {                   
        ...
        $cartList = M('Cart as a')->join('ty_store as b on a.store_id=b.store_id','LEFT')->join('ty_goods as  c on a.goods_id=c.goods_id','LEFT')->field('a.*,b.is_own_shop,c.exchange_integral')->where($where)->select();// 获取购物车商品
        ...
        $total_price = array('total_fee' =>$total_price , 'cut_fee' => $cut_fee,'num'=> $anum,'exchange_integral'=>$exchange_integral); // 总计  
    ...
    {        
        ...
        {                           
                ...
                $data = array(
                        ...
                        'integral_money'   =>number_format($car_price['store_point_count'][$k] / tpCache('distribut.point_rate'),2,'.',''),//'使用积分抵多少钱',
                        'gift_jifeng'      =>$car_price['store_point_count'][$k], // 赠送使用的积分数量
                        'gift_jifeng_money'=>number_format($car_price['store_point_count'][$k] / tpCache('distribut.point_rate'),2,'.',''),//'赠送使用积分抵多少钱',
                ...
                $order_rate=tpCache('distribut.order_rate')*0.01;//获取佣金比例
                foreach($cartList as $key => $val)
                {
                   ...
                   $data2['gift_jifeng_money']  = number_format($val['goods_price']*$goods['give_integral']*0.01,2,'.','');     // 购买商品赠送积分/这里是钱没有换转为积分
                   $data2['integral_money']     = number_format($val['goods_price']*$goods['exchange_integral']*0.01, 2,'.',''); // 购买商品抵扣积分 /这里是钱没有换转为积分
				   
                   $data2['prom_type']          = $val['prom_type']; // 0 普通订单,1 限时抢购, 2 团购 , 3 促销优惠
                   $data2['prom_id']            = $val['prom_id']; // 活动id
                   $data2['store_id']           = $val['store_id']; // 店铺id
                   $data2['distribut']          = number_format($val['goods_price']*$order_rate, 2,'.',''); // 全额返佣比例
				   $data2['distributa']         = number_format(($val['goods_price']-$data2['integral_money'])*$order_rate, 2, '.', '');// 抵扣返佣比例
                ...
                // 如果使用了积分或者余额才记录
				if($car_price['store_balance'][$k]){
					accountLog($user_id,-$car_price['store_balance'][$k],"user_money","购物下单",0,$order_id,$order_sn);
				}
				if($car_price['store_point_count'][$k]){
					accountLog($user_id,-$car_price['store_point_count'][$k],"pay_points","购物下单",0,$order_id,$order_sn);
				}
