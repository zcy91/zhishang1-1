...
//select $val,username,add_time,consult_type,content,parent_id from `tp_goods_consult`  where goods_id = 95)");
    ...
    public function index(){
        ...
        //print_r($aa);
        // 如果是手机跳转到 手机模块
        if(true == isMobile()){//true == isMobile()
    ...
    public function street()
    {
        ...
        $store_list = $store_logic->getStoreList($sc_id,5);
    ...
    public function store_qrcode(){
		...
		if(I('t_id')){
			setcookie('t_id', I('t_id'), time()+3600*24*7, '/');
			if(I('shang')=='jia'){
			  $u_info=M('users')->where("user_id=".I('t_id')." and level=3")->getField('user_id');	
			  if($u_info){setcookie('shang','jia', time()+3600*24*7, '/');}
			}
		 }
		$url = "https://www.yjy141319.com/index.php?s=/Mobile/Store/index/store_id/$store_id.html";
    	\QRcode::png($url);
    }
