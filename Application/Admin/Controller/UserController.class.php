...
class UserController extends BaseController {
    ...
    public function detail(){
        ...
        if(IS_POST){
            ...
            if($password != '' && $password != $password2){exit($this->error('两次输入密码不同'));}
	    ...
	    $zupu=explode(",",substr($user["zupu"],0,-1));
	    unset($zupu[0]);//删除的标识没用id
	    unset($zupu[count($zupu)]);//删除自己标识
	    array_splice($zupu,1,0);//重新索引
        $user['second_leader']=$zupu?$zupu[0]:0;
        $user['first_count'] = M('users')->where("zupu like '{$user['zupu']}%' and zuceng=".($user['zuceng']+1))->count();
        $user['second_count'] =M('users')->where("zupu like '{$user['zupu']}%' and zuceng=".($user['zuceng']+2))->count();
        $user_class = D('user_level')->select();
    ...
    public function delete(){
        $uid = I('get.id');
        $row = M('users')->where("user_id=$uid and user_id!=1")->delete();
        if($row){
            $this->success('成功删除会员');
        }else{
            $this->error('操作失败');
    ...
    public function account_edit(){
        ...
        if(IS_POST){
			...
			$frozen_act_type = I('post.frozen_act_type');
            $money = $user_money* $frozen_act_type;
            $desc = I('post.desc');
            if(!$desc)
                $this->error("请填写操作说明");
            if(accountLog($user_id,$money,$m_op_type,$desc)){
    ...
    public function user_pic()
    {
		...
		$ceng= I('ceng',3);
		if(strlen($key)==11){$key=about_xin("users","mobile=".$key,"user_id","user_id");}
	    $this->assign("ceng",$ceng);
		$this->assign("key",$key);
	    $this->assign("moue_list",moue_list($ceng,$key));
        $this->display();
    }
