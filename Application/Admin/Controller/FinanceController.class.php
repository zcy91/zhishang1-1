...
class FinanceController extends BaseController {
    ...
    public  function editStoreWithdrawals(){        
        ...
        if(IS_POST)
        {
                ...
                {                
                    ...
                    {
                        $this->error("店铺余额不足{$withdrawals['money']}，不够提现");    
                        exit;
                    }
                    storeAccountLog($withdrawals['store_id'], ($withdrawals['money'] * -1),0,$desc = '平台提现');
					accountLog(0,$withdrawals['sxf'],'user_money',"商家提现",$withdrawals['money'],$withdrawals['id'],'会员提现平台获利手续费');
    ...
    public  function editWithdrawals(){        
        ...
        if(IS_POST)
        {
                ...
                {                
                    ...
                    accountLog($withdrawals['user_id'], ($withdrawals['money'] * -1),'user_money',"会员提现",$withdrawals['fee'],$withdrawals['id'],'提现成功扣除'.$withdrawals['sxf'].'手续费');
					accountLog(0,$withdrawals['sxf'],'user_money',"会员提现",$withdrawals['money'],$withdrawals['id'],'会员提现平台获利手续费');
                    $remittance = array(
    ...
    public function order_statis_detail()
    {
        $create_date = I('create_date');
        $create_date = str_replace("+"," ",$create_date);
        $create_date2 = $create_date ? $create_date : date('Y-m-d', strtotime('-1 month')) . ' - ' . date('Y-m-d', strtotime('+1 month'));
        $create_date3 = explode(' - ', $create_date2);
		$create_date3[0]=strtotime($create_date3[0]);
		$create_date3[1]=strtotime($create_date3[1]);
		if(I('date_s') && I('date_e')){
			$create_date3[0]=I('date_s');
			$create_date3[1]=I('date_e');
		}
        $where = array(
            'store_id' => I('store_id'),
            'pay_status' => 1,
            'add_time' => array(array('gt', $create_date3[0]), array('lt', $create_date3[1])),
        );
