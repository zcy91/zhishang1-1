...
class StoreController extends BaseController{
	...
	public function store_info_edit(){
		...
		if($store_id>0){
			$store = M('store')->where("store_id=$store_id")->find();
			$store['lng_lat'] = $store['lng']&&$store['lat']?$store['lng'].','.$store['lat']:'';
			$this->assign('store',$store);
	...
	public function apply_list(){
		...
		$count = $model->where($map)->count();
		$Page = new \Think\Page($count,10);
		$list = $model->where($map)->order('apply_state asc')->limit($Page->firstRow.','.$Page->listRows)->select();
			...
			if(M('store_apply')->where("id=".$data['id'])->save($data)){
				if($data['apply_state'] == 1){
					...
					$store = array('user_id'=>$apply['user_id'],'seller_name'=>$apply['seller_name'],
							...
							'store_time'=>time(),'store_state'=>1,'store_qq'=>$apply['store_person_qq'],'store_user_name'=>$apply['store_person_name'],
                            'lng'=>$lng,'lat'=>$lat,
					);
					$store_id = M('store')->add($store);//通过审核开通店铺
                    $User = M("users");
                    $User->is_distribut=1;
                    $User->where(array('user_id'=>$apply['user_id']))->save();
					if($store_id){
