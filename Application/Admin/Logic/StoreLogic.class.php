...
{    
    ...
    public function addStore($store)
    {
		...
		if($user_id && $store_id && $seller_id){
			...
			if($pay_points > 0){
				accountLog($user_id, 0,$pay_points, '赠送积分'); // 会员注册赠送积分
			}
    ...
    public function auto_transfer($store_id){
	   ...
	   $end_day=mktime(0,0,0,date('m'),date('d'),date('Y'));//今日0点
 	   $start_day=mktime(0,0,0,date('m'),date('d')-tpCache('shopping.auto_transfer_date'),date('Y'));//昨日0点
            
        $sql = "select order_id from __PREFIX__order where store_id = $store_id and order_status in(2,4) and confirm_time >= $start_day and confirm_time < $end_day and is_checkout = 0";//
        ...
        $data = array(
            'start_date' => $start_day, // 结算开始时间
            'end_date'   => $end_day-1, // 结算截止时间
        ...
        {               
            $order_settlement = settlement_order($val['order_id']); // 调用全局结算方法

            $data['order_totals']      += $order_settlement['goods_price'];// 订单商品金额    
			...
			$data['give_money']        +=  $order_settlement['give_money'];// 送出积分金额
           
		    $data['deduction_integral']+=  $order_settlement['deduction_integral'];//抵扣积分
			$data['deduction_money']   +=  $order_settlement['deduction_money'];//抵扣金额
