...
class GoodsController extends MobileBaseController {
    ...
    public function goodsList(){
    	...
    	$id = I('get.id',M('goods_category')->where("1=1")->getField("id")); // 当前分类id 服务上要改为：849
    	$brand_id = I('brand_id',0);
    	//$spec = I('spec',0); // 规格
    	$attr = I('attr',''); // 属性
    	$sort = I('sort','goods_id')=='is_new'?'goods_id':I('sort','goods_id'); // 排序
    	...
    	if($count > 0)
    	{
			...
			$good_where_a="goods_id in (".  implode(',', $filter_goods_id).")";
			if(I('get.act')=='jifen'){$good_where_a=$good_where_a." and exchange_integral>0";}
    		$goods_list = M('goods')->where($good_where_a)->order("$sort $sort_asc")->limit($page->firstRow.','.$page->listRows)->select();
    ...
    public function goodsInfo(){
        ...
        $arr=array(
          "image_url"=>$goods['original_img']
        );
        array_unshift($goods_images_list,$arr);
       // var_dump($goods_images_list);exit;
        $goods_attribute = M('GoodsAttribute')->getField('attr_id,attr_name'); // 查询属性
        $goods_attr_list = M('GoodsAttr')->where("goods_id = $goods_id")->select(); // 查询商品属性表                        
		$filter_spec = $goodsLogic->get_spec($goods_id);
         
        $spec_goods_price  = M('spec_goods_price')->where("goods_id = $goods_id")->getField("key,key_name,price,store_count,img"); // 规格 对应 价格 库存表
...
//var_dump($filter_spec);exit();
        ...
        $this->assign('filter_spec',array_reverse($filter_spec));//规格参数
...
//        $show = $page->show();
    ...
    public function search(){
		if(I('post.searchtype')==1 and I('post.q')!=""){
		  $this->redirect('Index/street', array('key' => I('post.q')),0, '页面跳转中...');
		  exit;	
		}
    	$filter_param = array(); // 筛选数组
