...
class IndexController extends MobileBaseController {
     ...
     * 重新定位
     * */
    public function againLocation (){
        session('lng_lat',null);
        header("location:".U('index/nearby_store'));
    }
    /**
     * 店铺街
...
//print_r($store_list);exit;
	...
	public function navigation(){
        ...
        $myLatLng=I('myLatLng',session('lng_lat'));
        $myLatLng=explode(',',$myLatLng);
        $myLat=$myLatLng[1];
        $myLng=$myLatLng[0];
        $this->assign('myLat',$myLat);
        $this->assign('myLng',$myLng);
        $this->assign('shopLat',$shopLat);
        $this->assign('shopLng',$shopLng);
    ...
    public function  version(){
        ...
        $info=$version->where('id='.$id)->select();
        $this->ajaxReturn($info,'JSON');

    }
    public  function  dl(){
        $http = new \Org\Net\Http;
        $filename = "Public/apk/test.apk";
        $showname="apk";
