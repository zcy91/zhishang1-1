...
/*	public function agreement(){
    ...
    public function basic_info(){
       ...
       $type= M('store_class')->where(array('user_id'=>$this->user_id))->select();
        $this->assign('store_class',$type);
        $this->display();
    }
    public function  submission(){
        if($this->apply['apply_state'] == 1) redirect(U('Newjoin/apply_info'));
       if(empty($this->apply) || $this->apply['apply_state'] == 2){
            $data = I('post.');
