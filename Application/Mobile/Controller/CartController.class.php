...
class CartController extends MobileBaseController {
    ...
    public function cart2()
    {
        ...
        if(I('get.cart')){
             M('Cart')->where('user_id="'.$this->user_id.'"')->setField(array('selected'=>0));
			 M('Cart')->where('user_id='.$this->user_id.' and session_id="'.$this->session_id.'" and goods_id='.I('get.cart'))->setField(array('selected'=>1));
		}
		...
		$store_id_all = M('cart')->where("user_id = {$this->user_id} and selected = 1")->group('store_id')->getField('store_id',true); // 获取所有店铺id   
		$zying_store_id = M('cart as a')->join('ty_store as b on a.store_id=b.store_id')->where("a.user_id = {$this->user_id} and a.selected = 1 and b.is_own_shop=1")->Field('a.store_id')->group('a.store_id')->select(); // 获取所有自营店铺id     

//        if((count($store_id_all)>count($zying_store_id) and count($zying_store_id)<>0 ) or (count($zying_store_id)>1 ) ){
//		   $this->error ('自营店商品须单独下单，不可与别店商品一起下单', U('Mobile/Cart/cart'));
//		} 
    ...
    public function cart3(){
		...
		$order_goods = M('cart as a')->join('ty_store as b on a.store_id=b.store_id','LEFT')->join('ty_goods as  c on a.goods_id=c.goods_id','LEFT')->field('a.*,b.is_own_shop,c.exchange_integral')->where("a.user_id = {$this->user_id} and a.selected = 1")->select();
    ...
    public function cart4(){
        ...
        if($master_order_sn){                       
            $order = M('Order ')->where("master_order_sn = '$master_order_sn' and pay_status!=1")->find();
			$order['total_amount']=M('order')->where("master_order_sn = '$master_order_sn'")->sum('total_amount');
			$order['order_sn'] = $master_order_sn; // 临时修改 给支付宝那边去支付
			$jifeng = M('order_goods b')->field('sum((b.integral_money*b.goods_num)) as integral_money,sum(b.gift_jifeng_money*goods_num) as gift_jifeng_money')->where("b.order_id in(SELECT order_id                      AS tp_sum FROM ty_order WHERE (master_order_sn = ".$master_order_sn."))")->find();

            if(M('order')->where("master_order_sn = '$master_order_sn'")->sum('order_amount') == 0){                
                $order_order_list = U("Mobile/User/order_list");//U("Home/User/order_list");
                header("Location: $order_order_list");
            }           
        }else{
         $order = M('Order ')->where("order_id = $order_id")->find();
		 $jifeng = M('order_goods b')->field('sum((b.integral_money*b.goods_num)) as integral_money,sum(b.gift_jifeng_money*goods_num) as gift_jifeng_money')->where("b.order_id = $order_id")->find();
       ...
       // if(strstr($_SERVER['HTTP_USER_AGENT'],'MicroMessenger'))
           // $paymentList = M('Plugin')->where("type='payment' and status = 1 and code in('yue')")->select();//'weixin','cod'         
       // else 
            $paymentList = M('Plugin')->where("type='payment' and status = 1 and code!='alipayMobile' and  scene in(1)")->select();        
