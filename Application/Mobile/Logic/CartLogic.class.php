   ...
   return $freight;  
    ...
    public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price)
    {
        ...
        $data = array(
                ...
                'integral'         =>($car_price['pointsFee'] * tpCache('distribut.point_rate')), //'使用积分',
...
//        	M('Users')->where("user_id = $user_id")->setDec('pay_points',($car_price['pointsFee'] * tpCache('distribut.point_rate'))); // 用户的积分减 
//        if($car_price['balance']>0)//wei_end
//        	M('Users')->where("user_id = $user_id")->setDec('user_money',$car_price['balance']); // 用户的余额减         
        ...
        $data4['pay_points'] = -($car_price['pointsFee'] * tpCache('distribut.point_rate'));
        $data4['change_time'] = time();
        $data4['desc'] = '购物下单消费';
        $data4['order_sn'] = $order_sn;
        $data4['order_id'] = $order_id;    
        // 如果使用了积分或者余额才记录
        ($data4['user_money'] || $data4['pay_points']) &&accountLog($user_id,$data4['user_money'],$data4['pay_points'],$data4['desc'],0,$order_id,$order_sn);// M("AccountLog")->add($data4);//wei_end         
