    ...
    public function add_edit_withdrawals()
    {
        ...
        $withdrawals = $Model->where("id = $id")->find();
         ...
         {   
            ...
            $data = I('post.');
            $distribut_min = tpCache('distribut.sys_store_min_amount'); // 最少提现额度
			$bili = tpCache('distribut.sys_store_fee'); // 最少提现额度
			...
			$user_money=M('store')->where(array('store_id'=>STORE_ID))->getField('store_money');;
            if($data['money'] > $user_money){
                $this->error("你最多可提现{$user_money}账户余额.");
                exit;
            }
             if($withdrawals['status'] == 1){$this->error('申请成功的不能再修改');}
