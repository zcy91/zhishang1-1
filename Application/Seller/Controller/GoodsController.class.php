...
class GoodsController extends BaseController {
   ...
   /**
    ...
    public function addEditGoods(){   
            ...
            {                
                ...
                if(!$Goods->create(NULL,$type))// 根据表单提交的POST数据创建数据对象                 
                {
                   ...
                   // C('TOKEN_ON',true);                                                            
	                ...
	                if($Goods->exchange_integral > 100)
                        $this->ajaxReturn(json_encode(array('status' => -1,'msg'=> '抵扣'.tpCache('distribut.jifeng_name').'最高比例不得超过100%','data'  =>'')));				
	              
				    if($Goods->give_integral > 100)
                        $this->ajaxReturn(json_encode(array('status' => -1,'msg'=> '赠送'.tpCache('distribut.jifeng_name').'最高比例不得超过100%','data'  =>'')));				
                    ...
                    $return_arr = array(
                        'status' => 1,
                        'msg'   => '操作成功',                        
                        'data'  => array('url'=>'index.php?s=/Seller/Goods/goodsList/goods_state/1/is_back/1'),
                    );
            ...
            $this->assign('storeInfo',$store);  // 店主详情;
