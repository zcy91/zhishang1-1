...
class ReportController extends BaseController{
	...
	public function _initialize(){
		...
		if($timegap){
			...
			$lastweek = date('Y-m-d',strtotime("-15 day"));//30天前
	...
	public function index(){
		...
		$today['today_amount'] = M('order')->where("add_time>$now AND (pay_status=1 or pay_code='cod') and store_id=$this->store_id and order_status in(1,2,4)")->sum('order_amount+user_money');//今日销售总额
		...
		$sql = "SELECT COUNT(*) as tnum,sum(order_amount+user_money) as amount, FROM_UNIXTIME(add_time,'%Y-%m-%d') as gap from  __PREFIX__order ";
