	              ...
	              <table id="list-table" class="table table-bordered table-striped">
					...
					<tbody>
					  <?php if(is_array($cat_list)): foreach($cat_list as $k=>$vo): ?><tr role="row" align="center" class="<?php echo ($vo["level"]); ?>" id="<?php echo ($vo["level"]); ?>_<?php echo ($vo["cat_id"]); ?>">
	                     ...
	                     <td><img width="20" height="20" src="/Public/images/<?php if($vo[show_in_nav] == 1): ?>yes.png<?php else: ?>cancel.png<?php endif; ?>" onclick="changeTableVal('article_cat','cat_id','<?php echo ($vo["cat_id"]); ?>','show_in_nav',this)"/>
                         </td>
	                 	<td align="left">                         
                         ...
                         <td align="left" valign="middle"><?php echo 'http://'.$_SERVER['HTTP_HOST']; echo U('Mobile/Article/alist',array('lid'=>$vo['cat_id']));?></td>
	                     <td>
	                      <a class="btn btn-primary" href="<?php echo U('Article/category',array('act'=>'edit','cat_id'=>$vo['cat_id']));?>"><i class="fa fa-pencil"></i></a>
							 <?php if(in_array(($vo["id"]), is_array($article_system_id)?$article_system_id:explode(',',$article_system_id))): ?><a class="btn btn-default disabled" href="javascript:void(0)"><i class="fa fa-trash-o"></i></a><?php endif; ?>
							 <?php if(!in_array(($vo["id"]), is_array($article_system_id)?$article_system_id:explode(',',$article_system_id))): ?><a class="btn btn-danger" href="javascript:void(0)" data-url="<?php echo U('Article/categoryHandle');?>" data-id="<?php echo ($vo["cat_id"]); ?>" onclick="delfun(this)"><i class="fa fa-trash-o"></i></a><?php endif; ?>
						</td>
