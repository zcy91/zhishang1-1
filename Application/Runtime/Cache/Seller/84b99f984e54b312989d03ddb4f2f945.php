           ...
           <nav class="navbar navbar-default">	     
			<div class="collapse navbar-collapse">
                <div class="navbar-form pull-right margin">
                      <?php if($order['order_status'] < 2): ?><a href="<?php echo U('order/edit_order',array('order_id'=>$order['order_id']));?>" data-toggle="tooltip" title="" class="btn btn-primary" data-original-title="编辑">修改订单</a><?php endif; ?>
                      <?php if(($split == 1) and ($order['order_status'] < 2)): ?><a href="<?php echo U('order/split_order',array('order_id'=>$order['order_id']));?>" data-toggle="tooltip" title="" class="btn btn-primary" data-original-title="编辑">拆分订单</a><?php endif; ?>
                ...
                <table class="table table-bordered">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <?php if($order[shipping_status] == 1): ?><a href="<?php echo U('Order/shipping_print',array('order_id'=>$order['order_id'],'code'=>$order['shipping_code']));?>" target="_blank" class="btn btn-primary input-sm" onclick="">打印快递单</a><?php endif; ?>
                        </td>
            ...
            <div class="panel-body">
                <table class="table table-bordered">
                    ...
                    <?php if(is_array($orderGoods)): $i = 0; $__LIST__ = $orderGoods;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$good): $mod = ($i % 2 );++$i;?><tr>
                            <td class="text-left"><a><?php echo ($good["goods_name"]); ?></a></td>
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                <h3 class="panel-title text-center">费用信息
                    <a class="btn btn-primary btn-xs" data-original-title="修改费用" title="" data-toggle="tooltip" href="<?php echo U('Order/editprice',array('order_id'=>$order['order_id']));?>" style="display:none">
...
<?php if($order['fangshi'] == 1): ?><td class="text-right">赠送<?php echo tpCache('distribut.jifeng_name');?> (<?php echo ($order["gift_jifeng"]); ?>):</td>
<?php else: ?> 
                        <td class="text-right">抵扣<?php echo tpCache('distribut.jifeng_name');?> (-<?php echo ($order["integral"]); ?>):</td><?php endif; ?>
                        <td class="text-right"  style="display:none">余额抵扣</td>
                        <td class="text-right"  style="display:none">优惠券抵扣</td>
                        <td class="text-right"  style="display:none">价格调整:</td>
                        <td class="text-right">应付:</td>
                    </tr>
                    <tr>
                        <td class="text-right"><?php echo ($order["goods_price"]); ?></td>
                        <td class="text-right">+<?php echo ($order["shipping_price"]); ?></td>
<?php if($order['fangshi'] == 1): ?><td class="text-right"><?php echo ($order["gift_jifeng_money"]); ?></td>
<?php else: ?> 
                        <td class="text-right">-<?php echo ($order["integral_money"]); ?></td><?php endif; ?>                       
                        <td class="text-right"  style="display:none">-<?php echo ($order["user_money"]); ?></td>
                        <td class="text-right"  style="display:none">-<?php echo ($order["coupon_price"]); ?></td>
                        <td class="text-right"  style="display:none">减:<?php echo ($order["discount"]); ?></td>
                            ...
                            <td colspan="3">
                                <div class="input-group">
                                	<?php if(is_array($button)): foreach($button as $k=>$vo): if($k == 'pay_cancel'): ?><a class="btn btn-primary margin" href="javascript:void(0)" data-url="<?php echo U('Order/pay_cancel',array('order_id'=>$order['order_id']));?>" onclick="pay_cancel(this)"><?php echo ($vo); ?></a>
                                		...
                                		<?php else: ?>
                                		<button class="btn btn-primary margin" onclick="ajax_submit_form('order-action','<?php echo U('Order/order_action',array('order_id'=>$order['order_id'],'type'=>$k));?>');" type="button" id="confirm">
